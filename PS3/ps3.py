...
def update_hand(hand, word):
    ...
    for letter in hand.keys():
        if letter not in word:
            ...
            if hand[letter] - word.count(letter) > 0:
...
def is_valid_word(word, hand, wordlist):
    ...
    if '*' in word:
        ...
        for vowel in VOWELS:
            word_option = word
            word_option = word_option.replace('*', vowel)
            if word_option in wordlist:
...
def play_hand(hand, wordlist):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while len(hand) > 0:
        ...
        if word == '!!':
            return total_score  # End the game
            ...
            if is_valid_word(word, hand, wordlist):
                ...
                print(word, 'earned ', word_score, 'points.', end=" ")
    ...
    while num_hands > 0:
        ...
        if replay_flag == 1:
            ...
            if replay == 'yes':
                replay_flag = 0
                substitution_option_flag = 0
                hand_score = max(hand_score, play_hand(hand, wordlist))
                print('Total score for this hand: ', hand_score)
                print('---------------')

        num_hands = num_hands - 1
        total_series_score = total_series_score + hand_score

    print('Total score over all hands: ', total_series_score)

    return total_series_score
